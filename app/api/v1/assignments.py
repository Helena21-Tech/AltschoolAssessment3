...
async def submit_assignment(student_name: str = Form(...),
    ...
    new_assignment = await AssignmentService.submit_assignment(student_name, subject, description, file_path)
    await db.commit()
    return new_assignment


# Get all submitted assignments
@assignment_router.get("/",response_model=AssignmentRead ,status_code=200)
